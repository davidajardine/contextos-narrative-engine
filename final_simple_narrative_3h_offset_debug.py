...
# Configuration
# ---------------------------
DEBUG = True       # Set to False to run on the full dataset
SAMPLE_SIZE = 20  # Number of records to use in debug mode
...
def load_aggregated_data(db_path='bybit_data.db', table_name='new_aggregated_set_1year'):
    ...
    Convert trading_day to string, process the SND column, and create the shifted target SND_next.
    """
    conn = sqlite3.connect(db_path)
    df = pd.read_sql_query(f"SELECT * FROM {table_name}", conn)
    conn.close()
    # Convert trading_day to string for consistency.
    df['trading_day'] = pd.to_datetime(df['trading_day']).dt.date.astype(str)
    # Process SND column robustly.
    if 'SND' in df.columns:
        if not np.issubdtype(df['SND'].dtype, np.number):
            df['SND'] = df['SND'].astype(str).str.upper().map({'TRUE': 1, 'FALSE': 0})
    # Sort by trading_day and create SND_next target.
...
def compute_missing_metrics(df):
    """
    Compute additional metrics from the available raw OHLC and range values.
    For P2:
...
def bin_continuous_features(df, bin_config):
    ...
    If a column is missing or if there are insufficient unique values, log a warning and skip.
    """
    df_binned = df.copy()
...
# ---------------------------
def run_association_rule_mining(df, min_support=10, min_confidence=0.5, target_str='SND_next_1'):
    """
...
def main():
    ...
    # For debugging, check the loaded records (should be 367)
    ...
    # Define binning configuration including all desired continuous features.
    bin_config = {
        'daily_wick_pct': 3,
    ...
    logger.info("Continuous features binned.", sample_binned) # Sample binned columns: %s
    
    # Debug: Use a small sample if DEBUG is True.
    if DEBUG:
        logger.info("DEBUG mode active: Using a sample of %d records for association rule mining.", SAMPLE_SIZE)
        df_binned = df_binned.sample(n=SAMPLE_SIZE, random_state=42)
    
    # One-hot encode only categorical columns plus the binary target SND_next.
    # Convert SND_next to string so that one-hot encoding creates columns like SND_next_1.
    ...
    # Print encoded columns for verification
    ...
    # Save the rules to the database and CSV.
