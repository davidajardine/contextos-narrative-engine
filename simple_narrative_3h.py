...
print("Starting production script...")
...
# ---------------------------
# Compute extremes from 3‑h data for P1 and P2
...
features_for_rules = [
    ...
    'P2_set_daily_high',
    'P2_set_daily_low',
    'SND'                    # SND label from daily data (numeric 1/0)
]
...
# Retain day_of_week and SND as is; convert SND to string for one-hot encoding
df_enriched['SND'] = df_enriched['SND'].astype(str)

# Convert additional flag columns to strings for one-hot encoding
...
df_rules = pd.get_dummies(df_enriched, columns=[
    ...
    'P2_tookout_daily_low', 'P2_set_daily_high', 'P2_set_daily_low', 'SND'
], drop_first=False)
...
# Filter for rules with SND_True in the consequent (SND is now numeric, so 1 represents True)
rules_snd = rules[rules['consequents'].apply(lambda x: 'SND_1' in x)]
